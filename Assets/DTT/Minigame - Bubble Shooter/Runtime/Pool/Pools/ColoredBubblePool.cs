...
namespace DTT.BubbleShooter
{
	...
	/// <inheritdoc/>
	/// </summary>
	public class ColoredBubblePool : BubblePool
	{
		/// <summary>
		/// <inheritdoc/>
		/// </summary>
		...
		public ColoredBubblePool(HexagonGrid grid, float threshold) : base(grid, threshold) { }
		...
		protected override void OnRecompute()
		{
			...
			foreach(BubbleWrapper bubble in bubbles)
            {
				...
				Color color = ((ColoredBubble)bubble.Bubble).Color;
			...
			foreach(IEnumerable<BubbleWrapper> group in groups)
            {
				...
				p_sets.Add(new ColoredBubblePoolSet(bubbleGroup, chance));
            }
		}
    }
}